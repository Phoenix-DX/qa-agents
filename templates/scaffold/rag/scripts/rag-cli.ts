...
import { Embedder, SqliteStore, TwoStageRAG } from '../src/rag';
...
async function cmdQuery(question: string, collection: string, topN: number, asJson: boolean): Promise<void> {
  ...
  rows.forEach((row, i) => {
    ...
    console.log(divider);
    console.log(
      `[${i + 1}] ${row.source}  (vector: ${row.vectorScore.toFixed(3)}, rerank: ${row.rerankScore.toFixed(3)})`,
    );
    console.log(divider);
...
async function main(): Promise<void> {
  ...
  if (cmd === 'index') {
    ...
    const folder = positional[0] ?? DEFAULT_INDEX_FOLDER;
  ...
  console.log('Usage: rag-cli <index|query> ...');
  process.exit(1);
}
