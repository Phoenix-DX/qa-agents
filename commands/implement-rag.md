...
- `rag-cli: command not found`, or `npm error 404`/`403` resolving a `@scope/rag-cli`-shaped package (private RAG variant only) → this is a registry-auth problem, not something to solicit or fix in chat. The package is an optional dependency, so a token-less `npm install` skips it silently and this is the first place it shows up. Point to the project's `README.md` "RAG setup" section: add the registry token (preferably two lines in `~/.npmrc`, once per machine; or per-clone by copying `.npmrc.example` to `.npmrc`) and re-run `npm install` — it can't live in `.env.local`, since `npm install` needs it before any dotenv-loaded code runs. Then stop.
